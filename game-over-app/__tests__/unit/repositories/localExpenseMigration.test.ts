...
describe('localExpenseMigrationRepository', () => {
  ...
  it('reads both legacy keys, ignores category metadata, and preserves exact cents', async () => {
    ...
    expect(result).toEqual({ migratedExpenses: 2, alreadyCompleted: false });
    expect(JSON.parse(storage.get(stateKey)!)).toMatchObject({ completed: true });
  ...
  it('retries a failed write with the same IDs and never removes local data', async () => {
    ...
    expect(retry).toEqual({ migratedExpenses: 1, alreadyCompleted: false });
    expect(completed).toEqual({ migratedExpenses: 0, alreadyCompleted: true });
    expect(upsert).toHaveBeenCalledTimes(2);
    expect(upsert.mock.calls[0][0][0].id).toBe(upsert.mock.calls[1][0][0].id);
