...
`),
      ...
      function addStyles(style) {
        ...
        for (const style2 of cssStyles) {
          ...
          if (mediaTypeMatch) {
            const mediaType = mediaTypeMatch[1], mediaStyle = createMediaStyle(
              style2,
              mediaName,
      ...
      const onlyTernaries = attrs.flatMap((attr) => {
        ...
        return ternary.alternate && Object.keys(ternary.alternate).length && (mergedAlternate = (0, import_web.mergeProps)(
          mergeForwardBaseStyle || {},
          ternary.alternate || {}
        ), (0, import_propsToFontFamilyCache.forwardFontFamilyName)(ternary.alternate, mergedAlternate, baseFontFamily)), ternary.consequent && Object.keys(ternary.consequent).length && (mergedConsequent = (0, import_web.mergeProps)(
          mergeForwardBaseStyle || {},
          ternary.consequent || {}
      ...
      if (hasTernaries)
        for (const ternary of expandedTernaries) {
          ...
          const baseString = t.isStringLiteral(baseClassNameExpression) ? baseClassNameExpression.value : "", fullClassNameWithDups = (baseString ? `${baseString} ` : "") + classNames.join(" "), fullClassName = (0, import_concatClassName.concatClassName)(fullClassNameWithDups), classNameLiteral = t.stringLiteral(fullClassName);
      ...
      if (shouldPrintDebug && (console.info("attrs", JSON.stringify(attrs, null, 2)), console.info("expandedTernaries", JSON.stringify(expandedTernaries, null, 2)), console.info("finalExpression", JSON.stringify(finalExpression, null, 2)), console.info({ hasTernaries, baseClassNameExpression })), finalExpression) {
